"""
Motor de presupuesto WhatsApp: catálogo numerado + LISTO + IVA.
Sin IA: precios fijos del JSON del cliente.
"""
...
class MotorPresupuestoWhatsApp:
    """Motor de presupuesto por conversación (memoria por wa_id)."""

    def __init__(self, ruta_catalogo: Path | str) -> None:
...
def bloques_a_texto(bloques: list[str]) -> str:
    """Une bloques del motor en un solo mensaje WhatsApp."""
    partes = [b.strip() for b in bloques if (b or "").strip()]
    return "\n\n".join(partes)
